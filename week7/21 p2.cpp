...
int main() {
	...
	while (t--) {
		...
		*/
		...
		// 0번째는 루트이고, 루트는 이미 값이 저장돼있음 => i+1번째의 데이터를 건드리는 연산을 0부터 'n-1번' 수행함
		for (int i = 0; i < n - 1; i++) {
			...
			// ex) depth가 4 2라면 2(depth)의 부모인 1(depth)을 찾아서 insert
			// 현재, 다음 노드에 들어갈 값만 있고, 얘의 부모가 누군지는 알 수 없음 => 현재노드를 다음노드와의 깊이 차이만큼 ->parent 해서 알아내자
